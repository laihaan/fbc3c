...
// Giá của lệnh hunt9
const HUNT9_COST = 50000; // Có thể cần thay đổi giá nếu khác
const COOLDOWN_TIME = 5000; // 5 giây

// Hàm thực thi lệnh săn bắn 9 con
module.exports = {
    name: 'hunt9',
    execute: async (message, api) => {
        ...
        try {
            ...
            // Kiểm tra thời gian sử dụng lệnh hunt9
            const now = Date.now();
            if (!user.hunt9Timeout || now > user.hunt9Timeout) {
                // Xóa timeout khi hết thời gian
                user.hunt9Timeout = null;
                user.lastHunt9Time = null;
                await fs.writeFile(dataFilePath, JSON.stringify(data, null, 2));
                api.sendMessage('⏰ Thời gian sử dụng lệnh săn bắn 9 con đã hết. Bạn có thể mua thêm thời gian để tiếp tục.', message.threadID);
                return;
            }

            // Kiểm tra thời gian chờ giữa các lệnh
            if (user.lastHunt9Time && (now - user.lastHunt9Time < COOLDOWN_TIME)) {
                const remainingTime = Math.ceil((COOLDOWN_TIME - (now - user.lastHunt9Time)) / 1000);
                api.sendMessage(`⏳ Bạn phải chờ thêm ${remainingTime} giây trước khi thực hiện lệnh săn bắn 9 con tiếp theo.`, message.threadID);
                return;
            }

            // Cập nhật thời gian thực hiện lệnh
            user.lastHunt9Time = now;

            // Chọn ngẫu nhiên 9 con vật để săn
            const selectedAnimals = [];
            while (selectedAnimals.length < 9) {
